...
then
	...
	if [ -e "/etc/systemd/system/eibd.service" ]
	then
...
check_run ./configure --without-pth-test --enable-onlyeibd --enable-eibnetip --enable-eibnetiptunnel --enable-eibnetipserver --enable-groupcache --disable-usb --enable-ft12 --enable-tpuarts
check_run make
check_run sudo make install
...
# Create eibd  startup script
echo "Creating eibd startup script"
echo "*****************************************************************************************************"
echo "*                                 Creating eibd startup script                                      *"
echo "*****************************************************************************************************"
tee /etc/init.d/eibd <<- 'EOF'
#!/bin/sh
### BEGIN INIT INFO
# Provides:          eibd
...
do_init()
{
    ...
    if [ "$EIBD_BACKEND" = "AUTO" ]; then
        ...
        EIBD_NET_PORT=`grep Answer $EIBNETTMP | cut -d' ' -f6`
        ...
        if [ "$EIBD_NET_MCAST" != "" -a "$EIBD_NET_HOST" != "$EIBD_MY_IP" ]; then
            EIBD_BACKEND="ipt"
            EIBD_URL=$EIBD_NET_HOST:$EIBD_NET_PORT
            EIBD_PORT_IP=$EIBD_NET_PORT
            EIBD_PORT_IPT=$EIBD_NET_PORT
    ...
    echo "1- $EIBD_BACKEND"
    echo "2- $EIBD_NET_HOST:$EIBD_PORT_IPT"

    # concat urls
    if [ "$EIBD_BACKEND" = "ipt" -a -n "$EIBD_PORT_IPT" ]; then
        EIBD_URL=$EIBD_BACKEND:$EIBD_NET_HOST:$EIBD_NET_PORT
    fi
...
sudo update-rc.d eibd defaults 100 100
echo " " > /var/log/eibd.log
sudo chmod 777 /var/log/eibd.log

tee /etc/systemd/system/eibd.service <<- 'EOF'
[Unit]
Description=/etc/init.d/eibd Compatibility
After=syslog.target network.target
ConditionFileIsExecutable=/etc/init.d/eibd

[Service]
Type=oneshot
ExecStart=/etc/init.d/eibd start
ExecStop=/etc/init.d/eibd stop
TimeoutSec=0
...
EOF
chmod gu+xr /etc/systemd/system/eibd.service
sudo systemctl --system daemon-reload

sudo systemctl enable eibd.service
